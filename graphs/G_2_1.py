...
m = 1
G=nx.DiGraph()
fig1 = plt.figure(1, (n+3+m*0.2,n+1+m*0.25))
...
for u,v in G.edges():
    text = r"$\gamma_{{{},{}}}$".format(v[2],v[1]) if v != vend else ""
    ax.annotate("", pos[v],
                pos[u],
                ha="right", va="center",
    ...
    print((pos[v][1]+pos[u][1])/2)
    ax.text((pos[v][0]+pos[u][0])/2,(pos[v][1]+pos[u][1])/2+0.05, text, fontsize=15)
ax.xaxis.set_visible(False)
ax.yaxis.set_visible(False)
plt.draw()
plt.axis('off')
plt.savefig("G_2_1.png")
plt.show() # display
